    ...
    public partial class StartPage : PhoneApplicationPage
    {
        ...
        private async void LoginZH()
        {
            try
            {
                TopCoverFadeIn();
                isLogining = true;
                ZhihuAPI.Account account = new ZhihuAPI.Account();
                await account.Login(Mail.Text, PasswordBox.Password);
                NavigationService.Navigate(new Uri("/MainPage.xaml", UriKind.Relative));
            }
            catch
            {
                MessageBox.Show("", "登陆失败", MessageBoxButton.OK);
            }
            finally
            {
                TopCoverFadeOut();
        ...
        {
            var result = MessageBox.Show("暂不支持在客户端内注册，是否导航到知乎注册账号的网页？", "抱歉", MessageBoxButton.OKCancel);
        ...
        private void PhoneApplicationPage_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (!file.FileExists("style.css"))
            {
                StreamResourceInfo sr = Application.GetResourceStream(new Uri("Resources/style.css", UriKind.Relative));
                using (BinaryReader br = new BinaryReader(sr.Stream))
                {
                    byte[] data = br.ReadBytes((int)sr.Stream.Length);

                    using (BinaryWriter bw = new BinaryWriter(file.CreateFile("style.css")))
                    {
                        bw.Write(data);
                        bw.Close();
                    }
            ...
            if (!file.FileExists("ZHBackground.png"))
            {
                StreamResourceInfo sr = Application.GetResourceStream(new Uri("Images/ZHBackground.png", UriKind.Relative));
                using (BinaryReader br = new BinaryReader(sr.Stream))
                {
                    byte[] data = br.ReadBytes((int)sr.Stream.Length);

                    using (BinaryWriter bw = new BinaryWriter(file.CreateFile("ZHBackground.png")))
                    {
                        bw.Write(data);
                        bw.Close();
                    }
